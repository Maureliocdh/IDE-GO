...
Ingresa código Go-like y obtén el árbol sintáctico (AST) en consola.
"""
import sys
...
def imprimir_nodo(nodo, pad="", ultimo=True, etiqueta=""):
    ...
    if isinstance(nodo, list):
        ...
        for i, item in enumerate(nodo):
            ...
            imprimir_nodo(item, pad=pad_hijo, ultimo=es_ultimo, etiqueta=f"[{i}] ")
        return
    ...
    if hasattr(nodo, "__dict__"):
        ...
        # Recoger sólo campos con valor
        campos = [(k, v) for k, v in vars(nodo).items()
                  if v is not None and v != [] and v != {}]

        # Nodo "hoja" compacto: 1 campo con valor primitivo → una sola línea
        if len(campos) == 1:
        ...
        for i, (campo, valor) in enumerate(campos):
            es_ultimo = (i == len(campos) - 1)
            imprimir_nodo(valor, pad=pad_hijo, ultimo=es_ultimo,
...
def analizar(codigo: str):
    ...
    for t in tokens:
        print(f"  <{t.type.name:20} | {repr(t.value):20} | línea {t.line}>")
    ...
    campos_raiz = [(k, v) for k, v in vars(programa).items()
                   if v is not None and v != [] and v != {}]
...
def leer_multilinea():
    ...
    print("Ingresa el código Go-like y presiona Enter para analizar:")
...
def main():
    # Si se pasa un archivo como argumento: python sintactico.py archivo.go
    if len(sys.argv) > 1:
        ruta = sys.argv[1]
        try:
